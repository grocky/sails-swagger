...
const Transformer = {
  ...
  getDefinitionProperties (attributes, models) {

    return _.mapValues(attributes, (attr, attrName) => {
      let property = _.pick(attr, [
        'type', 'description', 'format', 'model', 'collection'
      ])

      let isNestedSchema = sails.config.blueprints.populate && property.model
      let isNestedCollection = sails.config.blueprints.populate && property.collection

      let properties
      if (isNestedSchema) {
        properties = { '$ref': Transformer.generateDefinitionReference(property.model)}
      } else if (property.model) {
        properties = Spec.getPropertyType(_.get(models[property.model], 'definition.id.type'))
        ...
        properties = {
          type: 'array',
          items: Spec.getPropertyType(_.get(models[property.collection], 'definition.id.type'))
        }
      } else {
        properties = Spec.getPropertyType(property.type)
      }
